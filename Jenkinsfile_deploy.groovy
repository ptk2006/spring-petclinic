            ...
            steps {
                script {
                    ...
                    withCredentials([sshUserPrivateKey(credentialsId: 'tls_private_key', keyFileVariable: 'SSH_KEY', usernameVariable: 'SSH_USER')]) {
                        sh 'for id in $(docker ps -q); do if [[ $(docker port "${id}") == *":80"* ]]; then docker stop "${id}"; fi; done'
                        sh "sleep 10; ssh -oStrictHostKeyChecking=no $SSH_USER@$INSTANCE -i $SSH_KEY docker run -d -p 80:8080 $imagename:$BUILD_VERSION"
                    }
                }
            }
        }
